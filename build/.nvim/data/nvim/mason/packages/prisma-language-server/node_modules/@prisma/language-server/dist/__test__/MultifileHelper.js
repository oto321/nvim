...
class MultfileHelper {
    ...
    file(filePath) {
        ...
        const doc = this.schema.findDocByUri(vscode_uri_1.URI.file(fullPath).toString());
        if (!doc) {
...
async function getMultifileSchema(folderPath) {
    const fullPath = path_1.default.join(multifileFixturesDir, folderPath);
    const files = await (0, schema_files_loader_1.loadSchemaFiles)(fullPath);
    ...
    return Schema_1.PrismaSchema.load(schemaDocs, fullPath);
}
